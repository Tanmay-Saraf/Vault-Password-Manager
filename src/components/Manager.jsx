...
const Manager = () => {
    ...
    let filtered = passwords.filter(item => item.url.toLowerCase().includes(search.toLowerCase()))
                ...
                <div className="rows flex flex-col w-full max-w-3xl mx-auto">
                    <div className='flex items-center  mb-6 justify-between'>
                        ...
                        <div className='relative flex justify-center items-center'>
                            ...
                            <input value={search} onChange={(e)=>setSearch(e.target.value)} type="text" placeholder='Search by URL' className='bg-neutral-950/50 border border-neutral-700 focus:border-red-500 focus:ring-1 focus:ring-red-500 outline-none w-full rounded-xl text-white py-1 px-10 text-sm transition-all duration-200'/>
                        </div>
                    </div>
                    {passwords.length === 0 && <div>No Passwords Stored yet!</div>}
                    {passwords.length !== 0 && filtered.map(item => {
