        ...
        const id = toast(
            <div>
                ...
                <div className="flex gap-2 mt-2">
                    <button className='px-3 py-1 bg-red-500 rounded-md' onClick={() => {
                        deletePass(item.id)
                        toast.dismiss(id)
                    }}>Yes</button>
                    <button className='px-3 py-1 bg-neutral-700 rounded-md' onClick={() => {
                        toast.dismiss(id)
                    }}>No</button>
                </div>
            </div>
            ...
            <div className="group flex justify-between items-center bg-neutral-950/30 hover:bg-neutral-950/50 border border-white/5 hover:border-white/10 transition-all duration-200 p-4 rounded-xl gap-4" >
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 w-full">
                    <div className="webs">
                        ...
                        <span className='text-white flex items-center gap-1'><span ><img className='rounded-full' height={"18px"} width={"18px"} src={`https://www.google.com/s2/favicons?domain=${item.url}`} /></span>{item.url}</span>
                        <span className='cursor-pointer' onClick={() => copyToClipboard(item.url)} title='Copy to clipboard'><i className="fa-solid fa-copy text-sm p-1 ml-1"></i></span>
                    </div>
                    <div className="webs">
                        <span className='text-xs text-neutral-500 block mb-1'>Username</span>
                        <span className='text-white'>{item.username}</span>
                        <span className='cursor-pointer' onClick={() => copyToClipboard(item.username)} title='Copy to clipboard'><i className="fa-solid fa-copy text-sm p-1 ml-1"></i></span>
                    </div>
                    <div className="webs">
                        ...
                        <div className='flex items-center gap-2'>
                            ...
                            <span className='cursor-pointer' onClick={() => copyToClipboard(item.password)} title='Copy to clipboard'><i className="fa-solid fa-copy text-sm p-1"></i></span>
                        </div>
                    </div>
                </div>
                <div className="buttons flex gap-3 justify-end items-center">
                    ...
                    <button title='delete' onClick={() => toaster()}>
                        <lord-icon
